...
coluna = st.sidebar.multiselect('Escolha as características:', fifa.drop(columns = ['Name', 'Teams','Values', 'Position', 'Foot']).columns)
data_client = {}
for col in coluna:
    data_client[col] = st.sidebar.number_input(f'Qual é o valor da coluna {col}?', 0, 100)
line_number = st.sidebar.number_input('Quantas recomendações você quer ?', 1, len(fifa))
...
if st.sidebar.checkbox('Recomendação'):
    ...
    if st.sidebar.checkbox('Gostaria de mostrar o gráfico?'):
